...
resource "aws_security_group" "external_alb_sg" {
  name        = "${var.project}-${var.env}-external-alb-sg"
...
resource "aws_security_group_rule" "external_alb_sg_ingress_http" {
...
resource "aws_security_group_rule" "external_alb_sg_ingress_https" {
  security_group_id = aws_security_group.external_alb_sg.id
  type              = "ingress"
  from_port         = 443
  to_port           = 443
...
resource "aws_security_group" "web_sg" {
  ...
  description = "HTTP and HTTPS from External ALB Security Group"
  vpc_id      = aws_vpc.vpc.id
...
resource "aws_security_group" "internal_alb_sg" {
  ...
  description = "HTTP and HTTPS from Web Security Group"
  vpc_id      = aws_vpc.vpc.id
...
resource "aws_security_group_rule" "internal_alb_sg_ingress_https" {
  security_group_id        = aws_security_group.internal_alb_sg.id
  type                     = "ingress"
  from_port                = 443
  to_port                  = 443
...
resource "aws_security_group" "app_sg" {
  ...
  description = "HTTP and HTTPS from Internal ALB Security Group"
  vpc_id      = aws_vpc.vpc.id
